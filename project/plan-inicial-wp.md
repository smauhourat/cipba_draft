...
7. [x] **Fix — el menú de mobile no abría bien (reportado por el usuario probando en un viewport angosto).** Causa: Astra y Max Mega Menu traen CADA UNO su propio sistema de menú responsive para la ubicación `primary`, y ambos usan el mismo breakpoint (1160px) pero no están integrados entre sí. Por debajo de ese ancho, Astra oculta su `#ast-desktop-header` completo (clase `ast-hide-custom-menu-mobile` en el `<header>`) — que es justo donde vive el botón hamburguesa real de Max Mega Menu, con Trámites/Normativa/etc. bien armados — y en su lugar muestra su propio `#ast-mobile-header`, que no tiene ningún menú de WordPress asignado a esa ubicación puntual del Header Builder y cae al listado de páginas por defecto de WordPress (roto: solo páginas sueltas, sin los desplegables ni la estructura real). Verificado con `curl` al HTML renderizado — el `<ul>` dentro de `#ast-hf-mobile-menu` mostraba `class="page_item page-item-81"` (fallback `wp_list_pages()`), no los ítems reales del menú Principal. **Fix:** en vez de intentar sincronizar los dos sistemas, se dejó a Max Mega Menu a cargo siempre (ya es responsive por sí solo) y se apagó el mobile menu propio de Astra por CSS: `@media (max-width:1160px){ #ast-desktop-header{display:block!important} #ast-mobile-header{display:none!important} }` en `style.css`. Pendiente de confirmación visual del usuario en mobile real (no solo devtools).
   - **Segundo fix, mismo hilo:** una vez visible, el menú mobile abría en modo acordeón (cada sección con flechita, colapsada, había que tocarla para desplegar) — el diseño real (comparado contra `project/index.html`) lo muestra siempre expandido de punta a punta, con "Trámites"/"Normativa" como encabezados en negrita sin flecha. Causa: Max Mega Menu trae un setting por ubicación de menú, `mobile_state`, con default `collapse_all` — no estaba seteado explícitamente. Se cambió a `expand_all` actualizando `megamenu_settings['primary']['mobile_state']` en la opción de la base (no hay UI de WP-CLI para esto, se hizo con `update_option` directo). Verificado con `curl` que el atributo `data-mobile-state="expand_all"` ya sale en el HTML — el comportamiento de ocultar la flechita y auto-expandir todo lo maneja el JS del propio plugin en base a ese atributo.
   - **Tercer fix, mismo hilo:** con el contenido ya bien expandido, el panel abría "pegado" al logo (encerrado en la misma columna angosta de la derecha del header) en vez de como un panel a todo el ancho debajo. Causa: Max Mega Menu arma el menú mobile expandido como una lista normal en flujo (`display:flex` en columna, dentro de su propio contenedor `#mega-menu-wrap-primary`), no como una capa flotante — así que hereda el ancho acotado de esa columna en vez de ocupar toda la pantalla. Se sacó del flujo por CSS: `#ast-desktop-header{position:relative}` + `#mega-menu-wrap-primary .mega-menu-toggle.mega-menu-open + #mega-menu-primary{position:absolute;top:100%;left:0;width:100%;...}` — ancla el panel al header completo (que sí ocupa el ancho total de la página), no a la columna del menú. Verificado que el CSS carga sin errores de sintaxis.
   - **Cuarto fix, mismo hilo:** el panel ya no quedaba pegado al logo, pero salía angostísimo (una columna de ~100px, el texto partido letra por letra). Causa: `position:absolute` con `top:100%` necesita que el contenedor con `position:relative` más cercano en el DOM sea el que se quiere usar como referencia (`#ast-desktop-header`) — pero algún contenedor intermedio de Astra entre medio (sticky header, probablemente) ya tenía su propio `position` seteado, así que el ancho se heredaba de ahí (angosto) en vez de del header completo. En vez de seguir adivinando cuál es exactamente ese contenedor, se resolvió con `position:fixed` (el contenedor de referencia pasa a ser directamente el viewport, sin ambigüedad de ancestros) + un script chico nuevo (`cipba_mobile_menu_panel_position_script()` en `inc/navbar.php`) que mide por JS el borde inferior real de `#ast-desktop-header` al tocar el botón hamburguesa y lo usa como `top` del panel — así no depende de adivinar la altura del header en CSS. Verificado que la home renderiza sin fatales y que el script sale en el HTML. Pendiente de confirmación visual — revisar en un celular real o F12 con "Dimensions" en un tamaño de teléfono.
